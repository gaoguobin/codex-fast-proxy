...
Report the JSON result. If the skill was newly linked or changed, explicitly tell the user:

```text
请重启 Codex App 并回到这个对话，或新开 CLI 实例，让它重新扫描 ~/.agents/skills；然后再说“启用 Codex Fast proxy”。
```

If a proxy process was already running before the update, do not claim that the running process has hot-reloaded. The update affects future manager commands and future proxy starts. Use `status` to report the current running state.
