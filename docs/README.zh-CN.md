...
Fast/Priority 是否真的加速取决于上游供应商支持情况，请以 A/B benchmark 结果为准。
...
- `请求记录`：最近 `/v1/responses`、Provider 检查和 benchmark 结果。
- `高级`：状态摘要、日志路径、自检、复制诊断、导出 JSON。

ChatGPT 账户登录态下，速度页会隐藏，因为速度选择由 Codex App 原生 UI 接管，概览里会显示 `App 控制`。
...
普通用户优先看 Control UI 的「高级」页。它会把运行时、配置入口、登录与密钥、启动钩子、请求日志和下一步操作合并成状态摘要，不需要先读原始 JSON。

`运行自检` 使用与 CLI `doctor` 相同的 manager 检查。`复制诊断` 和 `导出 JSON` 只包含脱敏状态，不包含 API key、Cookie、请求体、prompt、tool 参数或响应内容。
...
更新优先走 Control UI 右上角 `更新`。如果本地有未提交改动，更新会安全暂停，不会覆盖你的工作区。
