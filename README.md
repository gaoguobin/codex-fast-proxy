...
The default per-sample wall timeout is 300 seconds. If a full benchmark still reports timed-out
samples, rerun with a larger value such as `python -m codex_fast_proxy benchmark --timeout 600`.
...
默认每个样本的 wall timeout 是 300 秒。如果 full benchmark 仍然出现 timeout，可以用更大的值重跑，
例如 `python -m codex_fast_proxy benchmark --timeout 600`。`priority_accepted=true` 表示至少有一个
