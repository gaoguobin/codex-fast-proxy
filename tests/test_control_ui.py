...
class ControlUiTests(unittest.TestCase):
    ...
    def test_control_page_maps_preserve_policy_to_standard_speed_mode(self) -> None:
        ...
        self.assertIn('class="speed-mode-segments"', html)
        self.assertIn('class="speed-mode-option"', html)
        self.assertIn('class="secondary speed-save-button"', html)
        self.assertNotIn('compact-segments', html)
    ...
    def test_schedule_install_cleanup_unlinks_skill_uninstalls_package_and_delays_removal(self) -> None:
        ...
        self.assertIn("stop_control_ui_processes(app_home.parent)", command[2])
        self.assertIn("Get-CimInstance Win32_Process", command[2])
        self.assertIn("$ModuleNeedle", command[2])
        self.assertIn("$CodexHomeNeedle", command[2])
        self.assertNotIn("$Command.Contains('-m codex_fast_proxy ui')", command[2])
        self.assertNotIn("$Command.Contains('--codex-home')", command[2])
        self.assertIn("subprocess.TimeoutExpired", command[2])
